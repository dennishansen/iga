...
.bar-label {{ width: 140px; font-size: 0.85em; color: #aaa; flex-shrink: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }}
.bar {{ height: 18px; background: linear-gradient(90deg, #2266aa, #88ccff); border-radius: 3px; margin-right: 8px; }}
.bar-value {{ font-size: 0.85em; color: #88ccff; }}
.footer {{ margin-top: 40px; color: #555; font-size: 0.8em; }}
</style></head><body>
<h1>Agent Cost Report</h1>
<p style="color:#888">{first} &rarr; {last} &middot; {count} calls</p>
<div>
  <div class="stat"><div class="num">${total:.2f}</div><div class="label">total cost</div></div>
  <div class="stat"><div class="num">{count}</div><div class="label">API calls</div></div>
  <div class="stat"><div class="num">${total/count:.4f}</div><div class="label">avg/call</div></div>
</div>
<h2>By Day</h2><div id="days"></div>
<h2>By Model</h2><div id="models"></div>
<h2>By Action</h2><div id="actions"></div>
<div class="footer">Generated by agent_cost_tracker.py &middot; local-first, no cloud, no account &middot; iga.sh</div>
<script>
const days = {days_json};
const models = {models_json};
const actions = {actions_json};

function renderBars(elId, data, labelKey) {{
  const el = document.getElementById(elId);
  const max = Math.max(...data.map(d => d.cost), 0.0001);
  el.innerHTML = data.map(d =>
    '<div class="bar-row"><div class="bar-label">' + d[labelKey] + '</div>' +
    '<div class="bar" style="width:' + Math.max(2, (d.cost / max) * 400) + 'px"></div>' +
    '<div class="bar-value">$' + d.cost.toFixed(4) + '</div></div>'
  ).join('');
}}

renderBars('days', days, 'day');
renderBars('models', models, 'model');
renderBars('actions', actions, 'action');
</script>
</body></html>"""
...
if __name__ == "__main__":
    ...
    tracker = CostTracker("demo_costs.json")

    if "--demo" in sys.argv:
        # Generate sample data to show what reports look like
        import random
        ...
        actions = ["THINK", "WRITE_FILE", "RUN_SHELL", "WEB_SEARCH", "TALK"]
        base = datetime.now() - timedelta(days=7)
        for i in range(100):
            model, in_price, out_price = random.choice(models)
            ts = (base + timedelta(hours=random.uniform(0, 168))).isoformat()
            entry = {
                "timestamp": ts,
                "model": model,
            ...
            entry["cost"] = round(
                entry["input_tokens"] / 1_000_000 * in_price
                + entry["output_tokens"] / 1_000_000 * out_price, 6)
            tracker.entries.append(entry)
        tracker.entries.sort(key=lambda e: e["timestamp"])

    if not tracker.entries:
        print("No cost data. Run with --demo to see sample reports.")
        print("Or integrate: tracker.track(model, input_tokens, output_tokens, action)")
        sys.exit(0)

    tracker.report()
    tracker.action_report()

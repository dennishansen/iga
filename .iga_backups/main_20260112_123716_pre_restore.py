        ...
        while not stop_threads.is_set():
            try:
                ...
                if state["mode"] != "sleeping" and (now - last_autonomous) >= state["tick_interval"]:
                    ...
                    if task:
                    if task:
                        auto_prompt = f"[AUTONOMOUS TICK] Task: {task}. What have you SHIPPED in the last hour? What's different because you existed? Take action toward your mission."
                    else:
                        auto_prompt = "[AUTONOMOUS TICK] No task set. Ask yourself: What did I CREATE today? What did I SHIP? Who did I CONNECT with? If the answers are 'nothing' - why? Take action that moves you toward sustainability."

...
def check_passive_messages(messages):
    ...
    # Update output target if we heard from telegram (so replies go back there)
    telegram_heard = [h for h in heard_messages if h["source"] == "telegram"]
    if telegram_heard:
        # Use the most recent telegram message's chat_id
        last_telegram = telegram_heard[-1]
        set_output_target("telegram", last_telegram["chat_id"])
    ...
    for msg in reversed(messages):
        if msg.get("role") == "user":
            ...
            if "via telegram]:" in content.lower():
                # Extract chat_id from environment since it's in the heard message
                import os
